...
class PluginTest extends UplinkTestCase {
	...
	public function test_check_for_updates_with_same_results() {
		$this->assertSame(
			[],
			$this->resource->check_for_updates( [] ),
			'Return same transient if it is not an object'
		);

		$result = $this->resource->check_for_updates( new \stdClass() );
		$this->assertEquals( new \stdClass(), $result, 'Transient should remain same since we do not make an api call' );

		$update_from_option = get_option( 'stellar_uplink_update_status_' . $this->resource->get_slug() );
	...
	public function test_get_update_status() {
		$this->assertEquals( $this->expected_empty, $this->resource->get_update_status() );
		$update = new \stdClass();
		$time   = time();
		...
		update_option( 'stellar_uplink_update_status_' . $this->resource->get_slug(), $update );

		$this->assertEquals( $update,  $this->resource->get_update_status() );
	}

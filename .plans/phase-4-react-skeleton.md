 ...
 * Currently backed by resources/js/data/mock-plugins.json.
 *
 * @package StellarWP\Uplink
 */

/**
 * The license and activation state of a plugin entry.
 *
 * - active:       Licensed and the WP plugin is active.
 * - inactive:     Licensed but the WP plugin is either not activated or not yet
...
export interface Plugin {
    ...
    /**
     * URL to download and install the plugin zip.
     * Only present for inactive plugins that are not yet installed on this WordPress site.
     * When toggling to active, the REST API uses this URL to install the plugin first.
     */
    downloadUrl?: string;
    /**
...
export interface Brand {
    ...
    tagline: string;
    /** Plugins belonging to this brand */
    plugins: Plugin[];
}
...
- `downloadUrl` is optional on `Plugin` — present only for inactive plugins not yet installed. `upgradeUrl` is optional — present only for `not_included` plugins. This keeps the interface uniform without null fields on every entry.
